<?php
  ...
  if (isset($_POST["submit"])) {
    for ($pro_id = 1; $pro_id <= 5; $pro_id ++) {
      if (isset($_POST["".$pro_id])) {
        for ($ord_id = 1000*$_SESSION["UID"] + 1; $ord_id < 1000*($_SESSION["UID"] + 1); $ord_id ++) {
          if ($connection->query("SELECT * FROM `orders` WHERE `ord_id`=".$ord_id+1.";")->num_rows == 0) {
            break;
          }
        }
  ...
  if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
      ...
      echo "<img src='images/".$row["pro_id"].".png' width=150> </div>";
      echo "<form method=post><p>Amount:<input type=number name='".$row["pro_id"]."' placeholder='0'></p><p><p> <input type='submit' name='submit' value='Add to cart'> </p></p></form>";
    }
  } else {
    echo "0 results";
  }
